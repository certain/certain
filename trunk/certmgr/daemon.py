...
def create_daemon():
    ...
    try:
        ...
        # and inherits the parent's process group ID.  This step is required to
        # insure that the next call to os.setsid is successful.
        pid = os.fork()
    except OSError, e:
    ...
    if pid == 0:    # The first child.
        ...
        #  processes in the foreground process group of that terminal device.
        ...
        # dismiss. The process group is orphaned when the first child
        # terminates and POSIX.1 requires that every STOPPED process in an
        # orphaned process group be sent a SIGHUP signal followed by a SIGCONT
        # signal.  Since the second child is not STOPPED though, we can safely
        # forego ignoring the SIGHUP signal.  In any case, there are no
    ...
    # number of open file descriptors to close.  If it doesn't exists, use the
...
if __name__ == "__main__":
    ...
    """ % (os.getpid(), os.getppid(), os.getpgrp(), os.getsid(0), os.getuid(),
